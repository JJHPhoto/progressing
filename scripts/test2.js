...
function StepsList({chartGoal, loadSteps, setStep}) {
    ...
    useEffect(() => {
        const data = {
          [listId]: localStorage.getItem(listId) === 'true' ? true : false,
        };
        setCheckboxId(data);
      });
    ...
    const handleChange = (e) => {
        ...
        if (isChecked) {
            localStorage.setItem(listId, isChecked);
            toggleValue = true
      ...
      {chartGoal.steps.map((step) => {
        return (
                <Form className="checklist m-5">   
                    <Form.Check className="milestone-header" name={step.name} type="checkbox" style={{fontSize: "20px"}} id={step._id} value={step.complete} onChange={e => handleChange(e)} label={step.name} checked={currentCheckboxId.listId}/>   
                </Form>   
                )
            })} 
        </>
